...
pa.elev <- terra::rast("Data Management/Rasters/DEM/paDEM.tiff")
...
#' Extract elevation
nests.scaled.elev <- terra::extract(pa.elev, nests.sf)
nests.scaled$elev <- nests.scaled.elev$Layer_1
...
nests.scaled$elev <- scale(nests.scaled$elev)
...
nests.scaled$elev <- as.numeric(nests.scaled$elev)
...
write.csv(nest.sites, "Data Management/Csvs/Processed/Covariates/Pennsylvania/Weather/nests.sites.csv")
