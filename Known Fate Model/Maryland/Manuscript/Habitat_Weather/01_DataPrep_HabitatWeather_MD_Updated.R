...
# Nest 1214_2024_2 had an error with the coordinates
nests <- nests %>%
  dplyr::select(NestID, 
                BandID, 
                startI, 
...
# Atlantic covariates contain the separate rasters for each state merged together
md.nlcd <- terra::rast("Data Management/Rasters/NLCD/Atlantic/atlantic.nlcd.tif")
md.roads.prim <- terra::rast("Data Management/Rasters/Roads/Atlantic/atlantic.prim.tiff")
md.roads.sec <- terra::rast("Data Management/Rasters/Roads/Atlantic/atlantic.sec.tiff")
...
# Remove NAs associated with Atlantic ocean from analyses
nests.scaled <- na.omit(nests.scaled)
...
nest.sites <- nests.scaled.ready %>%
  ...
  st_drop_geometry()
write.csv(nest.sites,"Data Management/Csvs/Processed/Nests/Nests/nest.sites.weather_MD.updated.csv")
