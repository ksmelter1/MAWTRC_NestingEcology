...
#load("Data Management/RData/Known Fate/Maryland/20250409_PreliminaryResults_Habitat_Climate.RData")
...
f<k
 ...
 #' 4-19 to 9-16
 temperatureC2023 <- weather.array.copy[1:102,105:195,1]
 temperatureC2024 <- weather.array.copy[1:102,(105+366):(195+366),1]
 precip2023 <- weather.array.copy[1:102,105:195,2]
 precip2024 <- weather.array.copy[1:102,(105+366):(195+366),2]
 
 testTempC = data.frame(matrix(NA, nrow = 102, ncol = 90))   
 testPrecip = data.frame(matrix(NA, nrow = 102, ncol = 90))
...
temperatureC2023 <- weather.array.copy[1:102,1:365,1]
temperatureC2024 <- weather.array.copy[1:102,366:730,1]
precip2023 <- weather.array.copy[1:102,1:365,2]
precip2024 <- weather.array.copy[1:102,366:730,2]
 
 testTempC3Roll = data.frame(matrix(NA, nrow = 102, ncol = 90))   
 testPrecip3Roll = data.frame(matrix(NA, nrow = 102, ncol = 90))
 for(i in 1:nrow(surv.caps)){
 if(surv.caps$Year[i] == 1){
...
fill_NA_with_value <- function(df, value = 0) {
  df[] <- lapply(df, function(col) {
    if (is.numeric(col)) {
      col[is.na(col)] <- value  # Replace NA in numeric columns
    }
    return(col)
  })
  return(df)
...
save(p2.betas, samples_df, mean_estimates, file = "kf.betas.habitat.MD.RData")
