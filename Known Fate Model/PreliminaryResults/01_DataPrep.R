...
nests <- read_csv("Data Management/Csvs/Processed/IncubationDates/Draft3/20250131_NestAttempts_allbirds.csv")
nests
...
nests.scaled$age <- ifelse(nests.scaled$age == "A" & nests.scaled$yrsincecap >= 1, "A", nests.scaled$age)
glimpse(nests.scaled)
...
all_files <- list.files(path = "Data Management/Rasters/Weather/Rasters/Daily_5km_Rasters/")

# Filter files that match the pattern
filtered_files <- all_files[grepl("daily_min_temp_5km_all_PA_day_", all_files)]
...
tmin_5k_full <- lapply(sorted_files, function(x) terra::rast(paste0("Data Management/Rasters/Weather/Rasters/Daily_5km_Rasters/", x)))

# Conduct the extraction for the observations. (This step takes the longest)
out=lapply(c(1:length(tmin_5k_full)), function(x)
  
  terra::extract(tmin_5k_full[[x]], nests.sf, df=T))
...
tmin=data.frame(test) # Change to dataframe
colnames(tmin)=paste0("day", c(1:length(tmin_5k_full))) # Column names are the days since January first 2017
...
# Load the rasters using lapply
precip_5k_full <- lapply(sorted_files, function(x) terra::rast((paste0("Weather Covariates/Scripts/Scripts/Weather_Modeling/2_Spatial_Data_Ready_To_Use/DayMet_5KM_Rasters/", x))))

# Conduct the extraction for the observations. (This step takes the longest)
out=lapply(c(1:length(precip_5k_full)), function(x)
  
  terra::extract(precip_5k_full[[x]], nests.sf, df=T))

# Move the list of data per day to a data frame of data per date (columns) per trap (rows)
test=do.call(cbind, lapply(out, function(x) x[,2]))
precip=data.frame(test) # Change to dataframe
colnames(precip)=paste0("day", c(1:length(precip_5k_full))) # Column names are the days since January first 2022

precip[precip < 0] = NA  # Replace negative precipitation values with NA (optional)


#' Account for rainfall and create a binary indicator
# Step 1: Convert precipitation from millimeters to centimeters
precip.bin = precip / 10

# Step 2: Assign 1 if the value is greater than or equal to 0.254 cm, otherwise assign 0
precip.bin[precip.bin >= 0.254] = 1
precip.bin[precip.bin < 0.254] = 0
