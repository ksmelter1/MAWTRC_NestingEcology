...
load("Data Management/RData/Known Fate/New Jersey/01_Covs_Ready_Updated.RData")
...
# Median incubation date
nests.scaled$startI <- as.Date(nests.scaled$startI)
median(nests.scaled$startI, na.rm = TRUE)
...
nests.scaled.ready %>% group_by(NestYr) %>% summarise(mindate = min(startI),
                                                maxdate = max(endI)) %>% st_drop_geometry()
inc.dates <- sort(unique(c(nests.scaled$startI, nests.scaled$endI)))
inc.dates <- seq(as.Date("2024-04-08"), as.Date("2024-06-01"), by = 1)
...
f <- apply(surv.caps[,1:55],1,getFirst)
k <- apply(surv.caps[,1:55],1,getLast)
f;k
f<k
...
testTempC3Roll = data.frame(matrix(NA, nrow = 12, ncol = 55))   
testPrecip3Roll = data.frame(matrix(NA, nrow = 12, ncol = 55))
...
#' Check work
testTempC3Roll[1,1:55]
testPrecip3Roll[1,1:55]
...
# Check for NAs
summary(nests.scaled.ready)
nests.ready <- nests.scaled.ready
...
samples_matrix <- as.matrix(nimbleMCMC_samples[, 1:12])
