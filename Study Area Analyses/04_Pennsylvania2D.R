...
colMeans(nimbleMCMC_samples[, 24926:24933])
colSds(nimbleMCMC_samples[, 24926:24933])

# View traceplots
MCMCtrace(nimbleMCMC_samples[, 24926:24933], pdf = F)

# Extract the posterior samples for the 'beta' parameters (columns 219 to 230)
beta_samples <- nimbleMCMC_samples[, 24926:24933]
...
samples_matrix <- as.matrix(nimbleMCMC_samples[, 24926:24933])
...
save(mean_estimates1,
     samples_df,
     file = "MAWTRC Nesting Ecology Manuscript/Figures/RData/Pre-Nesting Movement/Study Area Analysis/pre.2D.updated.RData")
...
pa.nests.covs.2 <- pa.nests.covs.2[-c(405:587), ]
...
colMeans(nimbleMCMC_samples[,83:98])
colSds(nimbleMCMC_samples[,83:98])

# Extract the posterior samples for the 'beta' parameters (columns 219 to 230)
beta_samples <- nimbleMCMC_samples[, 83:98]
...
# Force conversion to plain matrix
samples_matrix <- as.matrix(nimbleMCMC_samples[, 83:98])
...
save(mean_estimates2,
     samples_df,
     file = "MAWTRC Nesting Ecology Manuscript/Figures/RData/Pre-Nesting Movement/Study Area Analysis/NSS.2D.updated.RData")
...
nests.scaled <- nests.scaled[-c(83:265), ]
...
weather.array.copy[1,731:1095,2]
...
temperatureC2022 <- weather.array.copy[1:82,1:365,1]
temperatureC2023 <- weather.array.copy[1:82,366:730,1]
temperatureC2024 <- weather.array.copy[1:82, 731:1095,1]
precip2022 <- weather.array[1:82,1:365,2]
precip2023 <- weather.array[1:82,366:730,2]
precip2024 <- weather.array.copy[1:82, 731:1095,2]
...
testTempC3Roll = data.frame(matrix(NA, nrow = 82, ncol = 144))   
testPrecip3Roll = data.frame(matrix(NA, nrow = 82, ncol = 144))
...
# We had NAs in the land cover variables
...
save(mean_estimates3,
     samples_df,
     file = "MAWTRC Nesting Ecology Manuscript/Figures/RData/Pre-Nesting Movement/Study Area Analysis/kf.2D.updated.RData")
