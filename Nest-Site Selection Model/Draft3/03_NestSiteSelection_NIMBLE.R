...
#' Load in RData
load("Data Management/RData/Nest-Site Selection/Covs/Draft3/20250118_Nests.Covs.RData")
...
nest.data <- nest.data %>%
  dplyr::select(nestid, bandid, avrgmxv, grssfrb, percwdy,
                case, Developed, Deciduous, Mixed, Evergreen, Agriculture,
                primary, secondary, basal, Native, Invasive, Other, Grassland)
...
colMeans(nimbleMCMC_samples[,145:158])
colSds(nimbleMCMC_samples[,145:158])

#' Extract the posterior samples for the 'beta' parameters (columns 219 to 230)
beta_samples <- nimbleMCMC_samples[, 145:158]
...
##########################################
## Save RData file 20250118_NimbleResults
