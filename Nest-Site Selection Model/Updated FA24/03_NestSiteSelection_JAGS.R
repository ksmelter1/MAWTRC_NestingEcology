...
nest.data$woodytyp <- ifelse(nest.data$woodytyp %in% c("Decid. Eric", "decid. ericaceous", "Deciduous ericaceous"), yes = "Ericaceous",
                             no = ifelse(nest.data$woodytyp %in% c("evergreen shrub", "Evrgrn Shrub"), yes = "EvergreenShrub",
                                         no = ifelse(nest.data$woodytyp %in% c("grpvns dwn/dead wood", "Grpvines dwn/dead wood"), yes = "Grapevines/DWD",          
...
nest.data$Ericaceous<- ifelse(nest.data$woodytyp  == "Ericaceous", 1, 0)
nest.data$EvergreenShrub <- ifelse(nest.data$woodytyp == "EvergreenShrub", 1, 0)
nest.data$Grapevines <- ifelse(nest.data$woodytyp == "Grapevines/DWD", 1, 0)
...
nest.data <- nest.data %>%
  dplyr::select(nest_id, bandid, avrgmxv, grssfrb, percwdy,
                ...
                Ericaceous, EvergreenShrub, Grapevines, Invasive)
...
X <- cbind(
  ...
  nest.data.ready$Ericaceous,      # Ericaceous
  nest.data.ready$Grapevines       # Grapevines/DWD
)
...
new_names <- c("Intercept", "Distance to Primary Road", "Distance to Secondary Road", 
               ...
               "Percent Woody", "Basal Area", "Invasive", "Ericaceous", "Grapevines/Down Woody Debris")
...
saveRDS(jags_samples, "01_NestModelSamples.Raw.RDS_12_9")
saveRDS(samples_df, "02_NestModelSamples.RDS_12_9")
